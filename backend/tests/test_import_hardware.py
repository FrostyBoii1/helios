...
def test_source_examples_not_matched_through_import(seeded):
    example = "ALPHA ESS M5 5KW INVERTER AND 15KW BATTERY"  # a curated source_example string
    parsed = {"details": {"_v": 2}, "inverter_raw": example}
    enrich_row_hardware(seeded, parsed)
    inv = parsed["details"]["hardware"]["inverters"][0]
    assert inv["model_text"] == example                      # preserved raw
